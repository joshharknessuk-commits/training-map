    ...
    <div className="min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-neutral-700">Find and book BJJ classes across London</p>
        </div>

        {Object.keys(groupedClasses).length === 0 ? (
          <div className="rounded-3xl border border-neutral-200 bg-white/70 p-8 text-center shadow-glow">
            <p className="text-neutral-600">No classes available at the moment.</p>
          </div>
        ) : (
            ...
            {Object.entries(groupedClasses).map(([day, dayClasses]) => (
              <div key={day}>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {dayClasses.map((cls) => (
                    <div key={cls.id} className="rounded-3xl border border-neutral-200 bg-white/70 p-6 shadow-glow">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h3 className="text-lg font-semibold text-neutral-900">{cls.name}</h3>
                          <p className="text-sm text-neutral-600 capitalize">{cls.classType.replace(/_/g, ' ')}</p>
                        </div>
                        <span
                      ...
                      {cls.description && (
                        <p className="text-sm text-neutral-600 mb-4">{cls.description}</p>
                      )}

                      <div className="space-y-2 mb-4">
                        {cls.instructorName && (
                          <p className="text-sm text-neutral-700">
                            <span className="text-neutral-500">Instructor:</span> {cls.instructorName}
                          </p>
                        )}
                        <p className="text-sm text-neutral-700">
                          <span className="text-neutral-500">Time:</span> {cls.startTime} - {cls.endTime}
                        </p>
                        <p className="text-sm text-neutral-700">
