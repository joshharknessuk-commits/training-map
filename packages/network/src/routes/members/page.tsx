...
export default function MembersPage() {
  return (
    <div className="space-y-10 rounded-3xl border border-white/10 bg-[#05070b] p-10 shadow-[0_25px_70px_rgba(0,0,0,0.5)]">
      <div className="space-y-3 text-left">
        <p className="text-xs font-semibold uppercase tracking-[0.5em] text-emerald-200">Members</p>
        <h1 className="text-4xl font-semibold tracking-tight text-white">One pass. Train everywhere.</h1>
        <p className="text-sm text-slate-300">
          London’s open mats become one big schedule. Scan your rotating QR and you’re on the mat.
        </p>
        <p className="text-xs text-slate-400">Built to strengthen grappling communities—not centralise them.</p>
      </div>

      <section className="space-y-3">
        <h2 className="text-lg font-semibold tracking-tight text-white">Why members love it</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold tracking-tight text-white">Pricing tiers</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold tracking-tight text-white">How the QR works</h2>
        <div className="grid gap-3 sm:grid-cols-3 text-sm text-slate-200">
          {[
            ...
            <div key={step.title} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
              <p className="text-[0.6rem] font-semibold uppercase tracking-[0.4em] text-emerald-200">{step.title}</p>
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2 text-sm text-slate-200">
          {MEMBER_FAQ.map((item) => (
            <details key={item.question} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
              <summary className="cursor-pointer text-white">{item.question}</summary>
